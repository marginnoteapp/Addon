JSB.newAddon = function(mainPath){
  JSB.require('SampleViewControllerC');
  var newAddonClass = JSB.defineClass('SampleCAddon : JSExtension', /*Instance members*/{
    //Window initialize
    sceneWillConnect: function() {
        ...
        self.sampleController = SampleViewControllerC.new();
    ...
    notebookWillOpen: function(notebookid) {
      NSTimer.scheduledTimerWithTimeInterval(0.2,false,function(){
        var sample_on = NSUserDefaults.standardUserDefaults().objectForKey('marginnote_sample_c_on');
    ...
    toggleSample: function(sender) {
      if(self.sampleController.view.window){
        ...
        NSUserDefaults.standardUserDefaults().setObjectForKey(false,'marginnote_sample_c_on');
      }
      else{
        Application.sharedInstance().studyController(self.window).view.addSubview(self.sampleController.view);        
        self.layoutViewController();
        NSUserDefaults.standardUserDefaults().setObjectForKey(true,'marginnote_sample_c_on');
        NSTimer.scheduledTimerWithTimeInterval(0.2,false,function(){ 

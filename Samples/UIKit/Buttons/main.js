JSB.newAddon = function(mainPath){
  JSB.require('SampleViewControllerU');
  var newAddonClass = JSB.defineClass('SampleUAddon : JSExtension', /*Instance members*/{
    //Window initialize
    sceneWillConnect: function() {
        ...
        self.sampleController = SampleViewControllerU.new();
    ...
    notebookWillOpen: function(notebookid) {
      NSTimer.scheduledTimerWithTimeInterval(0.2,false,function(){
        var sample_on = NSUserDefaults.standardUserDefaults().objectForKey('marginnote_sample_u_on');
    ...
    toggleSample: function(sender) {
      if(self.sampleController.view.window){
        ...
        NSUserDefaults.standardUserDefaults().setObjectForKey(false,'marginnote_sample_u_on');
      }
      else{
        Application.sharedInstance().studyController(self.window).view.addSubview(self.sampleController.view);        
        self.layoutViewController();
        NSUserDefaults.standardUserDefaults().setObjectForKey(true,'marginnote_sample_u_on');
        NSTimer.scheduledTimerWithTimeInterval(0.2,false,function(){ 
